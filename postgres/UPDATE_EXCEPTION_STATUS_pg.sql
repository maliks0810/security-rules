...
DROP FUNCTION IF EXISTS public."UPDATE_EXCEPTION_STATUS"(character varying, numeric, boolean);

-- Stamps MODIFIED_DATE / MODIFIED_BY on every EXCEPTION row matching
-- (ASSET_ID, RULE_ID). When p_complete is true, also flips STATUS_ID to 4
-- (Complete). When p_complete is false, status is left untouched — used
-- by ExecuteRules to record that the rule re-fired for an existing
-- (RuleID, AssetID) without changing its status.
