...
-- one row per BBG-compare rule. RULE_ID is resolved from the RULE table
-- by RULE_NAME so the Go ExecuteRule layer can tag each emitted
-- exception with its rule (it drops rows that lack RULE_ID).
