...
-- moved from rule names to exception ids. Snowflake overloads by
-- signature, so the old 3-arg definition has to be dropped explicitly
-- or it stays live alongside this one, still assigning by whole rule.
-- That DROP lives in 200_UPDATE_ADHOC.sql, which runs AFTER the
-- per-object files.
