...
const docTemplate = `{
    ...
    "basePath": "{{.BasePath}}",
    "paths": {
        "/v1/api/clearUserPreferences": {
            "post": {
                "description": "Removes the USER_PREFERENCES row for the (user,\nrule_group, rule_catalog) tuple so the grid falls back\nto its canonical default column order. Powers\nSettings → Reset Column Headers. rule_catalog empty /\nomitted = LHS tree at the group root (targets the row\nwhere RULE_CATALOG_ID IS NULL). Only that one scope is\ncleared; the user's other saved layouts are untouched.\nReturns {\"cleared\": 1} when a row was deleted and\n{\"cleared\": 0} when there was nothing to delete —\nboth are success, since \"no saved layout\" is the\nstate the caller is asking for.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                ...
                "parameters": [
                    {
                        "description": "user + rule_group + rule_catalog",
                        "name": "payload",
                        "in": "body",
                        "required": true,
                        "schema": {
                ...
                "responses": {
                    ...
                    "500": {
                        "description": "failed to clear user preferences",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
        ...
        "/v1/api/getUserPreferences": {
            "get": {
                "description": "Reads USER_PREFERENCES for the (user, rule_group,\nrule_catalog) tuple. rule_catalog empty / omitted =\nLHS tree at the group root (matches the row where\nRULE_CATALOG_ID IS NULL). Returns column_order as an\nopaque string (JSON array on the wire today, but the\nbackend doesn't parse it). Empty string when no\nsaved layout exists — client falls back to canonical\ndefault column order.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "parameters": [
                    {
                        "type": "string",
                        "description": "DM_USER.USER display name",
                        "name": "user",
                        "in": "query",
                        "required": true
                    },
                    ...
                    {
                        "type": "string",
                        "description": "RULE_CATALOG.NAME (empty for group-root scope)",
                        "name": "rule_catalog",
                        "in": "query"
                    }
                ],
        ...
        "/v1/api/refreshRulesByGroup": {
            "get": {
                "description": "Hits Snowflake for the requested scope via\nSP_GET_RULES_FOR_GROUP and rewrites the process-local\ncache slot(s) with the result — the next\n/getRulesForGroup call for the same group returns\nthe cached value without another Snowflake round-trip.\nCalled after DDL / seed changes touched the rule set\nso operators don't wait for the startup warm-up on\nthe next service restart.\nrule_group omitted / empty = re-warm every row in\nRULE_GROUP (bulk-invalidation path, same code path\nthe startup warmer runs).\nUses GET (mirrors /getRulesForGroup) even though it\nmutates in-process cache state — the underlying DB\nstays read-only, so the endpoint remains safe to\nretry and cache-bust from browsers / debug tooling.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "parameters": [
                    {
                        "type": "string",
                        "description": "RULE_GROUP.NAME (empty to refresh every group)",
                        "name": "rule_group",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "{refreshed: \u003ccount\u003e, rule_group: \u003cname\u003e?}",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "500": {
                        "description": "failed to refresh rules-by-group cache",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
        ...
        "/v1/api/refreshUserPreferences": {
            "get": {
                "description": "Hits Snowflake for the (user, rule_group,\nrule_catalog) tuple via SP_GET_USER_PREFERENCES and\nrewrites the process-local cache slot with the\nresult — the next /getUserPreferences call for the\nsame scope returns the cached value without another\nSnowflake round-trip. Called by the frontend\nimmediately after Save Column Order so the\njust-persisted layout is the cached one, and\navailable for any admin flow that suspects cache\ndrift. rule_catalog empty / omitted = LHS tree at\nthe group root (row where RULE_CATALOG_ID IS NULL).\nUses GET (mirrors /getUserPreferences) even though\nit mutates in-process cache state — the underlying\nDB stays read-only, so the endpoint remains safe to\nretry and cache-bust from browsers / debug tooling.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "responses": {
                    ...
                    "500": {
                        "description": "failed to refresh user preferences",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
        ...
        "/v1/api/updateUserPreferences": {
            "post": {
                "description": "Upserts USER_PREFERENCES for the (user, rule_group,\nrule_catalog) tuple. rule_catalog empty / omitted =\nLHS tree at the group root (RULE_CATALOG_ID stored\nas NULL). column_order is an opaque string the client\nencodes (JSON array of grid column names today).\nReturns {\"status\": 1} for insert, {\"status\": 2} for\nupdate, {\"status\": 0} for no-op (unknown user, group,\nor explicit catalog name).",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                ...
                "parameters": [
                    {
                        "description": "user + rule_group + rule_catalog + column_order",
                        "name": "payload",
                        "in": "body",
                        "required": true,
                        "schema": {
