...
BEGIN
    ...
    -- ASSET_ID with MIN_BY / MAX aggregates instead of the older
    -- FIRST_VALUE OVER + QUALIFY ROW_NUMBER pattern. MIN_BY is a streaming
    -- aggregate (no per-partition sort), so the plan is materially cheaper
    -- on Snowflake than the windowed equivalent. ALL_COMPLETE is derived
    -- from EXCEPTION_COUNT to avoid a second pass.
    res := (
        WITH filtered AS (
            SELECT
