-- GET_ASSETS now pulls FIGI from SECURITY_EXCEPTION.ID_BB_GLOBAL instead of
-- the previous hardcoded 'BBG00G6M2LZ2'. The outer DISTINCT ON picks the
-- latest row per asset, so FIGI tracks the most recent ID_BB_GLOBAL.
