-- Drop the previous mark-complete signature; that logic lives in
-- UPDATE_EXCEPTION_STATUS now.
DROP FUNCTION IF EXISTS public."UPDATE_SECURITY_EXCEPTION"(character varying, numeric);
...
-- Same parameter list as INSERT_SECURITY_EXCEPTION. Updates the existing
-- SECURITY_EXCEPTION row keyed by (ASSET_ID, RULE_ID) with the supplied
-- column values (ASSET_ID, RULE_ID, SECURITY_EXCEPTION_ID are not modified).
